...
class TerrainHeightCS : public ComputeShader {
    GLuint distanceBuffer;

public:
    TerrainHeightCS() {
        create("terrain_height.comp");
    ...
    void Dispatch(vec3 pos, float maxHeight, float& outGroundDist, GLuint segIndexSSBO, GLuint segIndexCount) {
        glUseProgram(getId());
